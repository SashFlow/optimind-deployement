	...
	>(
		...
		if (
			...
			this.pow2canvas.width =
				2 ** Math.floor(Math.log(image.width) / Math.LN2);
...
export function ReactShaderToy({
	...
	const initWebGL = () => {
		...
		glRef.current = (canvasRef.current.getContext(
			"webgl",
			contextAttributes,
		) ||
		...
		for (const uniform of Object.keys(uniformsRef.current)) {
			if (fragment.includes(uniform)) {
				...
				fragmentShader = insertStringAtIndex(
					...
					fragmentShader.lastIndexOf(precisionString) +
						precisionString.length,
				);
				u.isNeeded = true;
			}
		}
	...
	const setUniforms = (timestamp: number) => {
		...
		if (texturesArrRef.current.length > 0) {
			for (
				let index = 0;
				index < texturesArrRef.current.length;
				index++
			) {
