...
function getS3ClientFromConfig(cfg: Record<string, unknown>) {
	const endpoint = String(cfg.endpoint ?? process.env.S3_ENDPOINT ?? "");
...
export async function handleStorageS3(
	...
	const op = String(
		resolveValue(cfg.operation ?? "read", args.context),
	) as StorageOp;
	const bucket = String(
		resolveTemplate(
			String(cfg.bucket ?? process.env.S3_BUCKET_NAME ?? ""),
			args.context,
		),
	);
	const key = String(
		resolveTemplate(String(cfg.key ?? cfg.path ?? ""), args.context),
	);
	...
	if (op === "delete") {
		...
		return {
			kind: "success",
			output: { operation: op, bucket, key, deleted: true },
		};
	}
...
export async function handleStorageAzure(
	...
	if (op === "write") {
		...
		if (
			...
			throw new Error(
				"Azure write requires a SAS token in config.sasToken",
			);
		}
	...
	if (op === "delete") {
		...
		return {
			kind: "success",
			output: { operation: op, container, blob, deleted: true },
		};
	}
...
export async function handleStorageGcp(
	...
	if (op === "delete") {
		...
		return {
			kind: "success",
			output: { operation: op, bucket, object, deleted: true },
		};
	}
